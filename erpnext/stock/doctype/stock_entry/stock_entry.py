...
class StockEntry(StockController):
	...
	def opcost(self):
		doc=frappe.db.get_list('Process Order')
		for d in doc:
			doc1=frappe.get_doc('Process Order',d.name)
			if(self.process_order==d.name and self.purpose == 'Manufacture'):
				self.from_warehouse = doc1.src_warehouse
				...
				for d3 in self.get("items"):
					if str(d3.t_warehouse) == 'None':
						d3.s_warehouse = doc1.src_warehouse
	...
	def YeildValue(self,doctype):
		if self.stock_entry_type == 'Manufacture':
			...
			for s in self.get('items'):
				if str(s.t_warehouse) == 'None':
					tarWarQty=str(s.qty)
				if str(s.s_warehouse) == 'None':
					s.yielding = str((float(s.qty) / float(tarWarQty)) * 100)
				z = int(s.basic_rate)
				# s.basic_amt_cal = str((float(z) * float(s.yielding)) / 100)
				if str(s.yielding) == "None":
					s.basic_amt_cal = '0'
				else:
					pass
					# s.basic_amt_cal = str((float(z) * 100 ) / float(s.yielding))
		else:
			pass
	...
	def set_basic_rate(self, reset_outgoing_rate=True, raise_error_if_no_rate=True):
		...
		for d in self.get("items"):
			...
			elif d.is_finished_item and d.is_scrap_item:
				if self.purpose == "Manufacture":
